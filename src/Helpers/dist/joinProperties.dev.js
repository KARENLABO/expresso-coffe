...
function joinProperties(usersOrders, agroupedAmounts) {
  var mergedData = usersOrders.map(function (userData) {
    var user = userData.user;
    var totalAmountInfo = agroupedAmounts.find(function (item) {
      return item.user === user;
    });

    if (totalAmountInfo) {
      // If a corresponding entry is found in totalAmountData, add it
      return _objectSpread({}, userData, {
